...
library(ggplot2)
library(here)
folderpath = "fig 5 - NSCLC tumor analysis"
source(here(folderpath, "code", "spaceplot utils.R"))

#### data loading ----------------------------------------
name = "ICP20th"
# load segment/AOI annotation:
annot = read.table(here(folderpath, "data", paste0(name, "_SegmentProperties.txt")), 
...
xy = read.csv(here(folderpath, "data", "ICP20th_Lung11_grid_SampleSheet_20200207 - xy and nuclei.csv"))
...
geneannot = read.table(here(folderpath, "data", paste0(name, "_TargetProperties.txt")),
                       header = T, sep = "\t", quote = "", stringsAsFactors = F, comment.char = "")
...
# load raw counts:
raw = as.matrix(read.table(here(folderpath, "data", paste0(name, "_TargetCountMatrix.txt")),
                           header = T, row.names = 1))
raw[1:5,1:5]

# load normalized data:
negnorm = as.matrix(read.table(here(folderpath, "data", paste0(name, "_NegNorm_TargetCountMatrix.txt")),
                            header = T, row.names = 1))
...
svg(here(folderpath, "results", "supp figure - decon results with vs. without tumor.svg"), height = 10, width = 7.2)
print(g)
dev.off()
...
for (cell in immcells) {
  svg(here(folderpath, "results", paste0("cells in space granular - ", cell, ".svg")), width = 2, height = 2)
  par(mar = c(0,0,0,0))
...
svg(here(folderpath, "results", "florets - cells in space - legend.svg"), width = 4.75, height = 4)
par(mar = c(0,0,0,0))
# legend:
...
here(folderpath, "results", "florets - cells in space.svg")
par(mar = c(0,0,0,0))
...
dev.off()
svg(here(folderpath, "results", "decon heatmap - TME only.svg"), height = 3, width = 8)
pheatmap(pmin(res$cell.counts[p1$tree_row$order, use][, o], 100),
         col = viridis_pal(option = "B")(100),
         show_colnames = F,
         cluster_rows = F, cluster_cols = F)
...
svg(here(folderpath, "results", "decon barplot - props - TME only.svg"), height = 3, width = 8.1)
...
svg(here(folderpath, "results", "clusters in space.svg"), width = 5, height = 4)
